...
class ProductService {
  ...
  async updateProduct(req) {
    try {
      const prod = await Admin2db.product.findByIdAndUpdate(
        req.params.id,
        req.body,
        { new: true }
      );
  ...
  async deleteImage(req) {
    try {
      const prod = await Admin2db.product.findById(req.params.id);
      const img = prod.img.splice(
        prod.img.findIndex((i) => i === req.body.img),
        1
      );
  ...
  async contactUs(req) {
    try {
      ...
      await contactMail(
        ...
        con.mail,
        con.phno,
        con.date.toISOString().split("T")[0],
        con.message
      );
      await Admin2db.user.findByIdAndUpdate("66c2cb5300aa307e85b0bf20", {
        $inc: { contatusMail: 1 },
      });
      return con;
    } catch (err) {
  ...
  async Enquiry(req) {
    try {
      ...
      await dynamicMail(
        ...
        enq.mail,
        enq.phno,
        enq.date.toISOString().split("T")[0],
        enq.quantity,
        enq.message
      );
      await Admin2db.user.findByIdAndUpdate("66c2cb5300aa307e85b0bf20", {
